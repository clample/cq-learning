...
class CQLearner: 
    ...
    def __update_conflicting_states(self, new_global_state, new_local_state, reward):
    ...
    def __update_sliding_windows(self, new_local_state, reward):
        ...
        if not self.initial_rewards.get(new_local_state):
            self.initial_rewards[new_local_state] = { action:[] for action in self.possible_actions }

        if len(self.initial_rewards[new_local_state][self.previous_action]) < self.sliding_window_size:
            self.initial_rewards[new_local_state][self.previous_action].append(reward)
        else:
            if not self.latest_rewards.get(new_local_state):
                self.latest_rewards[new_local_state] = { action:[] for action in self.possible_actions }
